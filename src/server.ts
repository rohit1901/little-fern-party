...
export default class Server implements Party.Server {
    ...
    static async onBeforeConnect(req: Party.Request) {
        ...
        try {
            ...
            const JWKS = jose.createRemoteJWKSet(
                new URL(`${process.env.NEXT_PUBLIC_AUTH0_ISSUER}/.well-known/jwks.json`)
            );
    ...
    async onRequest(request: Party.Request) {
        ...
        if (request.method === "POST") {
            ...
            if (!payload.token) return new Response("Missing hCaptcha token", {status: 400});
            const data = await hCaptchaVerify(process.env.NEXT_PUBLIC_HCAPTCHA_SECRET, payload.token)
            if (data.success) {
            ...
            return new Response("Failed to verify hCaptcha token", {status: 400});
        }
